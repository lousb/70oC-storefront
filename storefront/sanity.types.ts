...
// Source: data/sanity/queries.ts
// Variable: HOME_SECTIONS_ORDER_QUERY
// Query: *[_type == 'home'][0].sections[].sectionName
export type HOME_SECTIONS_ORDER_QUERY_RESULT = Array<string | null> | null;
...
// Query: *[_type == "product" && defined(store.slug.current) && !store.isDeleted] | order(store.title asc) {    _id,    "title": store.title,    "slug": store.slug.current,    category,    "price": store.priceRange.minVariantPrice,    "imageUrl": coalesce(gallery[0].media.image.asset->url, store.previewImageUrl),  }
